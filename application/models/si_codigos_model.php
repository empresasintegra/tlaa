...
class Si_codigos_model extends CI_Model {
	...
	function buscar2($id){
	    $this->db->select('codigos_ccu as nombre');
		$this->from('codigos_ccu');
		$this->db->where('idcodigos_ccu', $id);
	    $query = $this->db->get();
		return $query->row();
	}
		...
		function id($id){
	    $this->db->select('codigos_ccu');
		$this->db->where('codigos_ccu.idcodigos_ccu', $id);
		$query = $this->db->get('codigos_ccu', 1)->row();
		return $query->codigos_ccu;
		}
