...
class Trabajador extends CI_Controller {
	...
	function guardar_persona(){
		...
		if ($si_existe_rut == 1) {
			...
			$auditoria_persona = array('tabla_id' => "personal_".$ultimo_id, 
				...
				'accion' => 1,
				'nombre'=>"Fue ingresado trabajador ".$_POST['nombre']." ".$_POST['apellido_paterno'].".",
	...
	function actualizar(){
		...
		//cargo nuevo
		$cargo_nuevo=$this->personal_model->listar8($id_cargo);
		//i.colectivo nuevo
		$i_colectivo_nuevo2=$this->personal_model->listar11($id_instrumento_colectivo);
		//empresa nuevo
        $id_empresa_nuevo=$this->personal_model->listar15($id_empresa);
		...
		//traer cargo actual
	    $id =$_POST['id'];
		$id_cargo_traer=$this->personal_model->listar5($id);
		$nombre_cargo=$this->personal_model->listar7($id_cargo_traer);
        
		//traer i.colectivo actual
		$i_colectivo_nuevo=$this->personal_model->listar9($id);
		$nombre_i_colectivo=$this->personal_model->listar10($i_colectivo_nuevo);

		//traer empresa actual
		$id_empresa_traer=$this->personal_model->listar13($id);
		$nombre_empresa=$this->personal_model->listar14($id_empresa_traer);
		...
		$nombre2=$this->personal_model->listar1($id);
		$rut2=$this->personal_model->listar2($id);
		$apellido_paterno2=$this->personal_model->listar3($id);
		$apellido_materno2=$this->personal_model->listar4($id);
	...
	if ($rut != $rut2){  
			...
			$auditoria_persona = array('tabla_id' => "personal_".$id, 
			   ...
			   'fecha' => date('Y-m-d G:i:s'),
			   'accion' => 2,
			   'nombre' =>"Rut ".$rut2." de trabajador ".$nombre." ".$apellido_paterno." "." fue actualizado a ".$rut.".",
			);
    ...
    if ($nombre != $nombre2){  
		    ...
		    $auditoria_persona = array('tabla_id' => "personal_".$id, 
			   ...
			   'fecha' => date('Y-m-d G:i:s'),
			   'accion' => 2,
			   'nombre' =>"Nombre trabajador ".$nombre2." ".$apellido_paterno." fue actualizado a ".$nombre." ".$apellido_paterno.".",
			);
	...
	if ($apellido_paterno != $apellido_paterno2){  
		    $this->personal_model->actualizar($id,$data);
		    ...
		    $auditoria_persona = array('tabla_id' => "personal_".$id, 
			   ...
			   'fecha' => date('Y-m-d G:i:s'),
			   'accion' => 2,
			   'nombre' =>"Ap.paterno trabajador ".$nombre2." ".$apellido_paterno2." fue actualizado a ".$nombre." ".$apellido_paterno.".",
			);
	...
	if ($apellido_materno != $apellido_materno2){  
		    $this->personal_model->actualizar($id,$data);
		    ...
		    $auditoria_persona = array('tabla_id' => "personal_".$id, 
			   ...
			   'fecha' => date('Y-m-d G:i:s'),
			   'accion' => 2,
			   'nombre' =>"Ap.materno trabajador ".$nombre2." ".$apellido_paterno2." ".$apellido_materno2." fue actualizado a ".$nombre." ".$apellido_paterno." ".$apellido_materno.".",
			);
		...
		$id_cargo=trim($_POST['id_select_cargo']);
		date_default_timezone_set('America/Santiago');
		 

		// ingresar nombre a tla cambio de cargo, convenio, empresa. -->comienzo
		if($nombre_cargo != $cargo_nuevo){
		$auditoria_trabajador = array('tabla_id' => "trabajador_".$id, 
			'usuario_id' => $this->session->userdata('id'),
			'fecha' => date('Y-m-d G:i:s'),
			'accion' => 2,
			'nombre'=>"Cargo trabajador ".$nombre." ".$apellido_paterno." se actualizo de ".$nombre_cargo." a ". $cargo_nuevo.".",
			);
		...
		if($nombre_i_colectivo != $i_colectivo_nuevo2){
			$auditoria_trabajador = array('tabla_id' => "trabajador_".$id, 
				'usuario_id' => $this->session->userdata('id'),
				'fecha' => date('Y-m-d G:i:s'),
				'accion' => 2,
				'nombre'=>"I.Colectivo trabajador ".$nombre." ".$apellido_paterno." se actualizo de ".$nombre_i_colectivo." a ". $i_colectivo_nuevo2.".",
				);
		...
		if($nombre_empresa!= $id_empresa_nuevo){
			$auditoria_trabajador = array('tabla_id' => "trabajador_".$id, 
				'usuario_id' => $this->session->userdata('id'),
				'fecha' => date('Y-m-d G:i:s'),
				'accion' => 2,
				'nombre'=>"Empresa trabajador ".$nombre." ".$apellido_paterno." se actualizo de ".$nombre_empresa." a ". $id_empresa_nuevo.".",
				);
		...
		// ingresar cambio de cargo, convenio, empresa. -->fin
	...
	function verificar(){
		...
		if (isset($_POST['eliminar'])){
			foreach($_POST['seleccionar_eliminar'] as $id){				
                ...
                //$id_trabajador=$_POST['seleccionar_eliminar'];
				//$buscar_persona=$this->personal_model->buscar_id_persona($id);
				$nombre=$this->personal_model->traer_nombre($id);
				$apellido=$this->personal_model->traer_apellido($id);
				...
				$auditoria_persona = array('tabla_id' => "personal_".$id, 
					...
					'fecha' => date('Y-m-d G:i:s'),
					'accion' => 3,
					'nombre' =>"Trabajador ".$nombre." ".$apellido." fue "."eliminado.",
				...
				$fecha_ahora = date('Y-m-d');
								$op = $this->personal_model->consulta_fecha_seleccionada2($fecha_ahora, $id);
								if($op = "SI"){
									$this->personal_model->actualiza_estado_trabajadores($datos3, $id, $fecha_ahora);
								}else{
									$this->personal_model->crea_nuevo_registro($datos3); 
								}
