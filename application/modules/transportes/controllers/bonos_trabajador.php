...
class Bonos_trabajador extends CI_Controller {
	...
	function calcular($fecha = FALSE, $fecha_2 = FALSE){
		...
		$base['cuerpo'] = $this->load->view('transportes/bonos_trabajador/calculo', $pagina, TRUE);
	...
	function calcular_bonos_trabajador(){

		$fecha_inicio = $_POST['mes'];
		$f = explode('-', $fecha_inicio);
		$ano = $f[0];
		$mes = $f[1];
		$dia = $f[2];

		if( $mes == 1 or  $mes == 3 or  $mes == 5 or  $mes ==  7 or  $mes ==  8 or  $mes ==  10 or  $mes ==  12  or $mes == 4 ){
			$dia= $dia + 30;
		}
		
		elseif($mes == 6 or  $mes ==  9 or  $mes ==  11){
			$dia= $dia + 29;
		...
		$fecha_termino = $ano.$conector.$mes.$conector.$dia;
		...
		//$this->tla_calcula_bono_produccion->dias_habiles($fecha_inicio,$fecha_termino); // 06-09-2019
