...
#1				#### REVISADO !
				if ($cargo == 72 and $convenio == 21) {
					...
					#### Calcula Vueltas
					if (!empty($fechas_produccion)){ //inicio if (!empty($fechas_produccion)){
						foreach ($fechas_produccion as $f) { // Inicio foreach ($fechas_produccion as $f) {
							$objPHPExcel->getActiveSheet()->SetCellValue('A'.$a, $f);
							...
							if($datos_produccion != NULL){ // inicio si $datos_produccion != null
								foreach ($datos_produccion as $chofer) {//inicio foreach
									if ($chofer->vuelta == 1) {
								...
								}// fin foreach ($datos_produccion as $chofer) {
							}//FIN if($datos_produccion != NULL){
						...
						//vuelta_adicional
							...
							$a++;
						}// FIN foreach ($fechas_produccion as $f) {
					}//FIN if (!empty($fechas_produccion)){
					...
					$objPHPExcel->getActiveSheet()->SetCellValue('C5', $fecha_actual);
					$s = 15;
					if (!empty($fechas_produccion)){ //Inicio If
						foreach ($fechas_produccion as $f) {//Inicio foreach ($fechas_produccion as $f)
					...
					$objWriter->save(BASE_URL2."extras/tla_excel/liquidaciones_tla/".$data->ap."_".$data->am."_".$rut.".xlsm");
				}
#/1
/*****************************************************************************************************************/
							...
							if($datos_produccion != NULL){ #inicio if 
								foreach ($datos_produccion as $chofer) {
									...
									if ($chofer->vuelta == 2) {
										...
										if ($chofer->tam_camion == 2) {
											//cajas
											$get_cajas = isset($datos_produccion_vuelta->cajas)?$datos_produccion_vuelta->cajas:'';
											if ($get_cajas==0) {
												$get_cajas = '';
											}	
											//bono_produccion				
...
/********************************************************************************************/
######################REVISADO
/*30-04-2018 incorporacion de vacio en vez de cero  desde la 2 vuelta en ($get_cajas)*/
								...
								foreach ($datos_produccion as $row => $values) {
									...
									elseif ($values->vuelta == 2) {
										...
										if ($values->tam_camion == 2) {		
											...
											$get_ayudantes = isset($datos_produccion_vuelta->tripulacion)?$datos_produccion_vuelta->tripulacion:'';
											if ($get_ayudantes==0) {
												$get_ayudantes = '';
											}
											$get_bono_produccion = isset($datos_produccion_vuelta->b_produccion)?$datos_produccion_vuelta->b_produccion:0;
...
/***********************************************************************************************************/
							#peoneta sindicato_nuevo /editado
/***********************************************************************************************************/
