	...
	function sube_excel($fecha = FALSE){
		...
		if ($si_existe == 1){
			...
			$tname  = $_FILES['dato']['tmp_name'];
				 ...
				 $formatos_permitidos = array('xlsx','xlx');
                 $extension = pathinfo($name, PATHINFO_EXTENSION);

 			if(!in_array($extension, $formatos_permitidos) ) {
   			 echo "<script>alert('Error formato no permitido !!')</script>";
   			 redirect('transportes/informes/resumen_preventa_rechazo', 'refresh');
			}else{
			...
			$sw=0;
					...
					$fecha1 =  $obj_excel->getActiveSheet()->getCell('D2')->getCalculatedValue();
		...
		if ($get_fecha == $fecha ){
            		...
            		$destino = BASE_URL2.'extras/archivos_preventas/'.$fecha.'-'.$_FILES['dato']['name'];
           
           				if( move_uploaded_file($_FILES['dato']['tmp_name'], $destino)){

           					$datos4 = array('archivo_excel' => $destino,
          					 'fecha_preventa' => $fecha,
          					 'fecha_ingreso' => date('Y-m-d')  );
           					$this->Si_sube_excel->probando($datos4);
        	...
        	echo "<script>alert('Fecha del archivo no coincide con fecha preventa !!')</script>";
           	redirect('transportes/informes/resumen_preventa_rechazo','refresh');
		...
		// redirect('transportes/informes/resumen_preventa_rechazo');
