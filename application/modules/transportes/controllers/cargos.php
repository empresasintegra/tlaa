...
class cargos extends CI_Controller {
	...
	function guardar_cargo(){
		...
		if (empty($nombre_add_cargo)) {
			...
			if ($si_existe == 1){
                ...
                $nombre=$this->cargos_model->buscar_registro($ultimo_id);
				...
				$auditoria_cargo = array('tabla_id' => "cargo_".$ultimo_id, 
					...
					'accion' => 1,
					'nombre'=>"Cargo ".$_POST['nombre']." fue ingresado.",
 					);
	...
	function actualizar(){			
		...
		if (empty($nombre_cargo)) {
            ...
            $nombre=$this->cargos_model->id($id);
			...
			$auditoria_cargo = array('tabla_id' => "cargo_".$id, 
					...
					'fecha' => date('Y-m-d G:i:s'),
					'accion' => 2,
					'nombre'=>"Cargo ".$nombre." fue actualizado a ".$_POST['nombre']."." ,
 			);
		...
		if (isset($_POST['eliminar'])){
			...
			foreach($_POST['seleccionar_eliminar'] as $id){	  		
				...
				$auditoria_cargo = array('tabla_id' => "cargo_".$id, 
					...
					'fecha' => date('Y-m-d G:i:s'),
					'accion' => 3,
					'nombre'=>"Cargo ".$nombre." fue eliminado.",
					);
