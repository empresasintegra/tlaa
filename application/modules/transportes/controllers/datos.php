...
//inicio modificaci¨®n 31-05-21 Edgard Urra - en caso de que apreten el boton guardar asistencia y todos tengan estado de inasistente
  ...
  if (isset($_POST['guardar'])){ 
    ...
    foreach($_POST['trabajadores'] as $c => $valores){
      if (!empty($_POST['trabajadores'][$c])) {           
        ...
        $datas2 = array( 'estado'=> $_POST['id_select_inasistencia'][$c]);
        if(in_array(7,$datas2)){
            $cont2 = $cont2 + 1;
            $cont1 = $cont1 + 1;
        }else{
          $cont1 = $cont1 + 1;
    ...
    if ($cont1 == $cont2){
      echo "<script>alert('No puede continuar, todos los trabajadores estan Ausentes')</script>";
...
// fin de modificaci¨®n 31-05-21 Edgard Urra
