$(document).ready(function(){
	var id_planta;
	var url_del = $("#eliminar_grupo").attr('href');
	var url_edit = $("#editar_grupo").attr('href');
	var volver = $("#eliminar_grupo").attr('rel');
	$("input[type=radio]").click(function(){
		id_planta =  $(this).val();
		url_final_del = url_del + '/' + id_planta + '/' + volver;
		url_final_edit = url_edit + '/' + id_planta;
		$("#eliminar_grupo").attr('href',url_final_del);
		$("#editar_grupo").attr('href',url_final_edit);
	});
	$("#eliminar_planta").click(function(event){
		...
		$.msgbox("¿Desea realmente eliminar esta planta?", {
		  ...
		  buttons : [
			...
			if(result){
				$.getJSON( base_url + 'administracion/grupos/ajax_validar_eliminar/'+id_planta , function(result){
